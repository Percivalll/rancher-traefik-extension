...
<required_reading>
@/workspace/.cursor/get-shit-done/references/domain-probes.md
@/workspace/.cursor/get-shit-done/references/gate-prompts.md
@/workspace/.cursor/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "/workspace/.cursor/get-shit-done/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@/workspace/.cursor/get-shit-done/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
